...
public class RunnerDeviceDeactivationTest {
    ...
    @Ignore
    @Test
    // Creates an device deactivation message check if its correctly created.
    public void createDeviceDeactivationMessage() throws ConnectionException {
        ...
        AbstractMessage message = ClientUtils.createDeactivation("", "register",
                "register_result", "client", "server",
                "0001", device.getUUIdentifier().toString(), "", device);


        assertTrue(((PayloadEntityDeactivation) message.getPayload()).reId.equals("0001"));
        assertEquals("{\"header\":{\"url\":\"\",\"topic\":\"register\",\"responseTopic\":\"register_result\",\"from\":\"server\",\"to\":\"client\",\"timeSent\":\"" + message.getTimeSent() + "\",\"messageType\":\"de.gtarc.chariot.deviceapi.payload.PayloadDeviceDeactivation\"},\"payload\":{\"reId\":\"0001\",\"uuid\":\"a1f114f5-f47f-449d-b579-6605bba061c8\",\"securitykey\":\"\"}}", message.getJsonObject());
    ...
    @Ignore
    @Test
    // Sends a device deactivation message to the server and then checks if its deactivated.
    public void deactivateDevice() throws ExecutionException, InterruptedException, TimeoutException, ConnectionException {
        Device device = new DeviceBuilder().buildSensingDevice();
        device.setUUIdentifier(UUID.fromString("a1f114f5-f47f-449d-b579-6605bba061c8"));
        ...
        registrationClient.deactivateEntity(dc,device, null, "").get();
